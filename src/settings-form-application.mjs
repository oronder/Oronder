...
export class OronderSettingsFormApplication extends FormApplication {
  ...
  bind() {
    if (this.object.guild && !this.form.elements.init) {
      ...
      this.object.guild.timezone = Array.from(this.form.elements.timezone).find(
        (c) => c.selected,
      ).value;
      ...
      if (
        ...
        this.object.guild.rollcall_role_id =
          Array.from(this.form.elements.rollcall_role).find((c) => c.selected)
            .value || undefined;
      ...
      this.object.players.forEach(
        (p) =>
          (p.discord_id =
            Array.from(this.form.elements[p.foundry_id].options).find(
              (o) => o.selected,
            )?.value ?? ""),
      );
    }
  ...
  async _init() {
    ...
    if (popup && !popup.closed && popup.focus) {
      ...
      Logger.error(
        "Could not launch Discord Authentication window! Check your browser's popup settings.",
      );
    }
