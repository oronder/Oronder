...
export function enrich_actor(actor) {
  ...
  const clone_spells = JSON.parse(
    JSON.stringify(actor.items.filter((item) => item.type === "spell")),
  ).map((spell) =>
    (({ name, img, system }) => ({
      name,
      img: fix_relative_url(img),
...
export const actor_to_discord_ids = (actor) =>
  Object.entries(actor.ownership)
    .filter(
      ([_, perm_lvl]) => perm_lvl === CONST.DOCUMENT_OWNERSHIP_LEVELS.OWNER,
    )
